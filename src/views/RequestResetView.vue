  ...
  <section class="vh-100 d-flex flex-column">
    <div class="mx-auto w-75">
      ...
      <form @submit.prevent="requestReset" class="w-100">
        ...
        <p v-if="success" class="text-white  text-start p-2 bg-success rounded-2 mb-4">На почту было отправлено письмо с дальнейшими инструкциями</p>

        <div class="mb-4">
          <input type="email" class="form-control" placeholder="Эл. почта" v-model="email">       
        </div>
...
export default({
  ...
  methods: {
    requestReset() {
      axios.post("auth/request_reset", {email: this.email})
        .then(() => {
          this.errors = null;
          this.success = true;
        })
